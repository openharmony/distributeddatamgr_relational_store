...
**SQLite 分层约束**：`SqliteConnection` 和 `SqliteStatement` 是唯一可以直接操作 SQLite API（持有 `sqlite3*`/`sqlite3_stmt*`）的类；`RdbStoreImpl`、`StepResultSet`、`SqliteSharedResultSet` 等上层类只通过 `Connection` 和 `Statement` 报表，**MUST NOT** 直接调用 sqlite3 系函数。
