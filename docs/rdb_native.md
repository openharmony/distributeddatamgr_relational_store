...
1. **连接池硬限制**: 永远不要超过 4 个连接 - 这是架构保证
2. **单写约束**: 同一时间只能有一个写操作，读操作可以并发
3. **事务作用域**: 避免长时间运行的事务，会阻塞其他写操作
4. **内存管理**: ResultSet 使用后必须清理，可能持有共享内存块

## 常见陷阱

- **内存泄漏**: ResultSet 使用后必须清理；它们可能持有共享内存块
- **连接生命周期**: 连接是池化的，不要手动关闭而不归还池
- **加密密钥**: HUKS 密钥丢失意味着加密数据库永久不可访问
- **跨进程访问**: RDB 文件可以跨进程访问，但写操作需要协调
