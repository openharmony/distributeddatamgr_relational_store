...
describe('gdbStorePromisePerf', () => {
    ...
    it('Perf_Gdb_Write_Promise_001', 0, async () => {
    ...
    it('Perf_Gdb_Write_Promise_002', 0, async () => {
        ...
        let startTime = new Date().getTime();
        for (let i = 0; i < BASE_COUNT; i++) {
            let UPDATE = "MATCH (n:Person {name: 'name_" + (i + 1) + "' }) SET n.age = " + (i + 2) + ";";
            await store.write(UPDATE);
        }
        let endTime = new Date().getTime();
        averageTime = ((endTime - startTime) * 1000) / BASE_COUNT;
        console.info(TAG + " the write_UPDATE_Promise average time is: " + averageTime + " μs");
        expect(averageTime < BASE_LINE).assertTrue();
        console.info(TAG + "************* testPerfGdbWritePromise002 end *************");
    })

    it('Perf_Gdb_Write_Promise_003', 0, async () => {
        ...
        let endTime = new Date().getTime();
        averageTime = ((endTime - startTime) * 1000) / BASE_COUNT;
        console.info(TAG + " the write_DELETE_Promise average time is: " + averageTime + " μs");
        expect(averageTime < BASE_LINE).assertTrue();
        console.info(TAG + "************* testPerfGdbWritePromise003 end *************");
    })

    it('Perf_Gdb_Read_Promise_001', 0, async () => {
