...
/**
 * @tc.name: RdbStoreConfig_040
 * @tc.desc: test RdbStoreConfig interfaces: SetModuleName/GetModuleName
 * @tc.type: FUNC
 */
HWTEST_F(RdbStoreConfigTest, RdbStoreConfig_040, TestSize.Level1)
{
    const std::string dbPath = RDB_TEST_PATH + "config_test_40.db";
    RdbHelper::DeleteRdbStore(dbPath);
    RdbStoreConfig config(dbPath);

    std::string bundleName = "com.ohos.config.test40";
    config.SetBundleName(bundleName);
    config.SetSecurityLevel(SecurityLevel::S1);
    config.SetArea(0);
    config.SetEncryptStatus(false);
...
/**
 * @tc.name: RdbStoreConfig_050
 * @tc.desc: test RdbStoreConfig interfaces: SetModuleName/GetModuleName
 * @tc.type: FUNC
 */
HWTEST_F(RdbStoreConfigTest, RdbStoreConfig_050, TestSize.Level1)
{
    const std::string dbPath = RDB_TEST_PATH + "config_test_50.db";
    RdbHelper::DeleteRdbStore(dbPath);
    RdbStoreConfig config(dbPath);
    ...
    auto invalidConfig = config;
    invalidConfig.SetEncryptStatus(true);
    store = RdbHelper::GetRdbStore(invalidConfig, 1, helper, errCode);
    EXPECT_EQ(store, nullptr);
    EXPECT_EQ(errCode, E_CONFIG_INVALID_CHANGE);
    store = nullptr;
