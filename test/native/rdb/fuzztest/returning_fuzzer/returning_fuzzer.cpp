...
static const int STRING_SIZE_MAX = 15;
...
std::vector<std::string> GetColumns(FuzzedDataProvider &provider, int max = 10, int min = 1)
{
    ...
    for (int i = 0; i < colCount; i++) {
        columns.push_back(provider.ConsumeRandomLengthString(STRING_SIZE_MAX));
    }
    return columns;
}
...
{
    std::string valName = provider.ConsumeRandomLengthString(STRING_SIZE_MAX);
...
void UpdateReturningFuzz(FuzzedDataProvider &provider, std::shared_ptr<RdbStore> store)
{
    ...
    predicates.EqualTo("name", ValueObject(provider.ConsumeRandomLengthString(STRING_SIZE_MAX)));
    ...
    predicates2.EqualTo("name", ValueObject(provider.ConsumeRandomLengthString(STRING_SIZE_MAX)));
...
{
    std::string sql = provider.ConsumeRandomLengthString(STRING_SIZE_MAX);

    std::vector<ValueObject> bindArgs;
    bindArgs.push_back(ValueObject(provider.ConsumeIntegral<int>()));
    bindArgs.push_back(ValueObject(provider.ConsumeRandomLengthString(STRING_SIZE_MAX)));
    bindArgs.push_back(ValueObject(provider.ConsumeIntegral<int>()));
...
void ValuesBucketFuzz(FuzzedDataProvider &provider)
{
    ...
    vb1.RowSize();
    std::string field = provider.ConsumeRandomLengthString(STRING_SIZE_MAX);
