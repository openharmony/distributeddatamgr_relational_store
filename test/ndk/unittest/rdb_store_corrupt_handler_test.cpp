...
void TestCorruptedHandler(OH_Rdb_ConfigV2 *config, void *context, OH_Rdb_Store *store)
{
    ...
    if (store == nullptr) {
        ...
        printf("test DemoDbCorruptedHandler %d\n", ret);
    } else {
        printf("DemoDbCorruptedHandler is start\n");
        int errCode = 0;
        errCode = OH_Rdb_Restore(store, restorePath1.c_str());
        printf("OH_Rdb_Restore is %d\n", errCode);
    }
}

void TestCorruptedHandler1(OH_Rdb_ConfigV2 *config, void *context, OH_Rdb_Store *store)
{
    std::string restorePath1 = "/data/storage/el2/database/com.ohos.example.distributedndk/entry/rdb/back_test.db";
    if (store == nullptr) {
        printf("test DemoDbCorruptedHandler repeat \n");
    } else {
        printf("repeat DemoDbCorruptedHandler is start\n");
    }
}
...
auto RDB_TEST_PATH1 = "/data/storage/el2/database/com.ohos.example.distributedndk/entry/rdb/rdb_store_test.db";
std::string backupPath1 = "/data/storage/el2/database/com.ohos.example.distributedndk/entry/rdb/back_test.db";
...
HWTEST_F(RdbStoreCorruptHandlerTest, RDB_Native_store_test_002, TestSize.Level1)
{
    ...
    errCode = OH_Rdb_Backup(store1, backupPath1.c_str());
    EXPECT_EQ(errCode, 0);
    EXPECT_EQ(OH_Rdb_ErrCode::RDB_OK, OH_Rdb_CloseStore(store1));
