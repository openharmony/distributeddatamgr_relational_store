...
| 命令 | 用途 | 备注 |
|------|------|------|
| `./build.sh --product-name <产品名> --build-target relational_store` | 构建整个组件 | **NEVER** 在 Agent 会话中运行 |
| `./build.sh --product-name <产品名> --build-target .../native_rdb` | 构建原生核心库 | |
| `./build.sh --product-name <产品名> --build-target .../test/native/rdb:unittest` | 构建原生测试 | |
| `clang-format -i <file>` | 格式化单个文件 | 提交前 **MUST** 格式化 |
...
| CI 任务 | 本地等价命令 |
|---------|-------------|
| 单元测试 | `./build.sh --product-name <产品名> --build-target .../test/native/rdb:unittest` |
| 格式化检查 | `clang-format -i <file>` |
| 编译检查 | `./build.sh --product-name <产品名> --build-target .../native_rdb` |
