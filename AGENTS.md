...
1. 修改存量接口 MUST 评估性能影响 — 运行性能基线测试，对照 BASE_LINE 阈值，改动后所有性能 case MUST 全 pass；涉及无现成基线的接口 MUST 先补基准测试再改代码
...
| `${OHOS_ROOT}/prebuilts/clang/ohos/linux-x86_64/llvm/bin/clang-format --style=file -i <file>` | 格式化单个文件 | `${OHOS_ROOT}` 为 OpenHarmony 源码根目录；提交前 **MUST** 格式化 |
