/*
* Copyright (c) 2023 Huawei Device Co., Ltd.
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
*     http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
*/

#ifndef RELATIONAL_VALUE_OBJECT_H
#define RELATIONAL_VALUE_OBJECT_H

/**
 * @addtogroup relationalStore
 * @{
 *
 * @brief RelationalStore module provides a series of external interfaces for insert data, delete data, update date,
 * and select data, as well as data encryption, hierarchical data protection, backup, and recovery functions.
 *
 * @syscap SystemCapability.DistributedDataManager.RelationalStore.Core
 * @since 10
 * @version 1.0
 */

/**
 * @file relational_value_object.h
 *
 * @brief Provides numeric type conversion functions.
 *
 * @since 10
 */

#include <cstdint>
#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Define OH_Rdb_VObject type.
 *
 * @since 10
 */
typedef struct {
    /** The id used to uniquely identify the OH_Rdb_VObject struct. */
    int64_t id;
} OH_Rdb_VObject;

/**
 * @brief Create an {@link OH_Rdb_VObject} instance.
 *
 * @return If the creation is successful, a pointer to the instance of the @link OH_Rdb_VObject} structure is returned,
 * otherwise NULL is returned.
 * @see OH_Rdb_VObject.
 * @since 10
 */
OH_Rdb_VObject *OH_Rdb_CreateValueObject();

/**
 * @brief Destroy the {@link OH_Rdb_VObject} object and reclaim the memory occupied by the object.
 *
 * @param valueObject Represents a pointer to an {@link OH_Rdb_VObject} instance.
 * @return Returns the status code of the execution. Successful execution returns RDB_ERR_OK,
 * while failure returns a specific error code. Specific error codes can be referenced {@link OH_Rdb_ErrCode}.
 * @see OH_Rdb_VObject, OH_Rdb_ErrCode.
 * @since 10
 * @version 1.0
 */
int OH_Rdb_DestroyValueObject(OH_Rdb_VObject *valueObject);

/**
 * @brief Convert the int64 input parameter to a value of type {@link OH_Rdb_VObject}.
 *
 * @param valueObject Represents a pointer to an {@link OH_Rdb_VObject} instance.
 * @param value Represents a pointer to an int64_t input parameter or the array of type int64_t.
 * @param count If value is a pointer to a single numerical value, count = 1;
 * if value is a pointer to an array, count is the size of the array.
 * @return Returns the status code of the execution. Successful execution returns RDB_ERR_OK,
 * while failure returns a specific error code. Specific error codes can be referenced {@link OH_Rdb_ErrCode}.
 * @see OH_Rdb_VObject.
 * @since 10
 * @version 1.0
 */
int OH_ValueObject_PutInt64(OH_Rdb_VObject *valueObject, int64_t *value, uint32_t count);

/**
 * @brief Convert the double input parameter to a value of type {@link OH_Rdb_VObject}.
 *
 * @param valueObject Represents a pointer to an {@link OH_Rdb_VObject} instance.
 * @param value Represents a pointer to an double input parameter or the array of type double.
 * @param count If value is a pointer to a single numerical value, count = 1;
 * if value is a pointer to an array, count is the size of the array.
 * @return Returns the status code of the execution. Successful execution returns RDB_ERR_OK,
 * while failure returns a specific error code. Specific error codes can be referenced {@link OH_Rdb_ErrCode}.
 * @see OH_Rdb_VObject.
 * @since 10
 * @version 1.0
 */
int OH_ValueObject_PutDouble(OH_Rdb_VObject *valueObject, double *value, uint32_t count);

/**
 * @brief Convert the char input parameter to a value of type {@link OH_Rdb_VObject}.
 *
 * @param valueObject Represents a pointer to an {@link OH_Rdb_VObject} instance.
 * @param value Indicates the const char * input parameter.
 * @return Returns the status code of the execution. Successful execution returns RDB_ERR_OK,
 * while failure returns a specific error code. Specific error codes can be referenced {@link OH_Rdb_ErrCode}.
 * @see OH_Rdb_VObject.
 * @since 10
 * @version 1.0
 */
int OH_ValueObject_PutText(OH_Rdb_VObject *valueObject, const char *value);

/**
 * @brief Convert the char * array input parameter to a value of type {@link OH_Rdb_VObject}.
 *
 * @param valueObject Represents a pointer to an {@link OH_Rdb_VObject} instance.
 * @param value Indicates the const char * array input parameter.
 * @param count Indicates the size of the value.
 * @return Returns the status code of the execution. Successful execution returns RDB_ERR_OK,
 * while failure returns a specific error code. Specific error codes can be referenced {@link OH_Rdb_ErrCode}.
 * @see OH_Rdb_VObject.
 * @since 10
 * @version 1.0
 */
int OH_ValueObject_PutTexts(OH_Rdb_VObject *valueObject, const char **value, uint32_t count);

#ifdef __cplusplus
};
#endif

#endif // RELATIONAL_VALUE_OBJECT_H
