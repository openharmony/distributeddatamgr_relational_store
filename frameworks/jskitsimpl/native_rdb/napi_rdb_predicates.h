...
public:
    ...
    const std::unique_ptr<NativeRdb::RdbPredicates> &getPredicates() const;
private:
    explicit RdbPredicatesProxy(std::string tableName);
    ~RdbPredicatesProxy();

    static napi_value New(napi_env env, napi_callback_info info);
    static std::unique_ptr<NativeRdb::RdbPredicates> &GetNativePredicates(napi_env env, napi_callback_info info);
    ...
    std::unique_ptr<NativeRdb::RdbPredicates> predicates_;
    napi_env env_;
    napi_ref wrapper_;
};
