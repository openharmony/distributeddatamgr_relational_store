...
void RdbPredicatesProxy::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor descriptors[] = {
        ...
        DECLARE_NAPI_FUNCTION("oOffset", Offset),
...
napi_value RdbPredicatesProxy::New(napi_env env, napi_callback_info info)
{
    ...
    if (is_constructor) {
        ...
        NAPI_ASSERT(env, valueType != napi_undefined, "Table name should be a string.");
...
{
    std::unique_ptr<RdbPredicates> predicates = std::make_unique<RdbPredicates>(tableName);
    predicates_ = std::move(predicates);
}

std::unique_ptr<NativeRdb::RdbPredicates> &RdbPredicatesProxy::GetNativePredicates(napi_env env,
...
const std::unique_ptr<NativeRdb::RdbPredicates> &RdbPredicatesProxy::getPredicates() const
{
    return predicates_;
}
} // namespace RdbJsKit
