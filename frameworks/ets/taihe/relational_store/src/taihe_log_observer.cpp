...
bool TaiheLogObserver::IsEquals(ani_ref ref)
{
    if (env_ == nullptr) {
...
int32_t TaiheLogObserver::AddCallback(RdbObserversData &rdbObserversData,
    JsExceptionMessageCallbackType callbackFunc, uintptr_t opq, SubscribeFuncType subscribeFunc)
{
    ...
    auto &observers = rdbObserversData.logObservers_;
    ...
    auto observer = std::make_shared<TaiheLogObserver>(env, aniRefHolder.move(), callbackPtr);
...
void TaiheLogObserver::RemoveCallback(RdbObserversData &rdbObserversData,
