    ...
    optional_view<ConflictResolution> conflict)
{
    ...
    ASSERT_THROW_INNER_ERROR(store != nullptr, OHOS::NativeRdb::E_ALREADY_CLOSED, "", returnVal);
    ASSERT_THROW_INNER_ERROR(RdbSqlUtils::IsValidTableName(std::string(table)),
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "Illegal table name", returnVal);
    auto buckets = ani_rdbutils::ValueBucketsToNative(values);
    ASSERT_THROW_INNER_ERROR(buckets.RowSize() != 0,
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "ValuesBuckets is invalid.", returnVal);
    ASSERT_THROW_INNER_ERROR(!RdbSqlUtils::HasDuplicateAssets(buckets),
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "Duplicate assets are not allowed", returnVal);
    ...
    ASSERT_THROW_INNER_ERROR(RdbSqlUtils::IsValidFields(nativeConfig.columns),
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "Illegal columns.", returnVal);
    ASSERT_THROW_INNER_ERROR(RdbSqlUtils::IsValidReturningMaxCount(nativeConfig.maxReturningCount),
    ...
    ASSERT_THROW_INNER_ERROR(errCode == OHOS::NativeRdb::E_OK, errCode, "", returnVal);
    ...
    weak::RdbPredicates predicates, ReturningConfig const &config, optional_view<ConflictResolution> conflict)
{
    ...
    ASSERT_THROW_INNER_ERROR(bucket.Size() != 0,
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "ValuesBucket is invalid.", returnVal);
    ASSERT_THROW_INNER_ERROR(!RdbSqlUtils::HasDuplicateAssets(bucket),
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "Duplicate assets are not allowed", returnVal);
    auto *impl = reinterpret_cast<RdbPredicatesImpl *>(predicates->GetSpecificImplPtr());
    ASSERT_THROW_PARAM_ERROR(impl != nullptr, "predicates", "an RdbPredicates.", returnVal);
    std::shared_ptr<OHOS::NativeRdb::RdbPredicates> rdbPredicateNative = impl->GetNativePtr();
    ASSERT_THROW_PARAM_ERROR(rdbPredicateNative != nullptr, "predicates", "an RdbPredicates.", returnVal);
    ASSERT_THROW_INNER_ERROR(RdbSqlUtils::IsValidTableName(rdbPredicateNative->GetTableName()),
        OHOS::NativeRdb::E_INVALID_ARGS_NEW, "Illegal table name", returnVal);
    auto nativeConfig = ani_rdbutils::ReturningConfigToNative(config);
    nativeConfig.columns = RdbSqlUtils::BatchTrim(nativeConfig.columns);
    ASSERT_THROW_INNER_ERROR(RdbSqlUtils::IsValidFields(nativeConfig.columns),
