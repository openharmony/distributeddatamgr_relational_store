...
int RdbStoreImpl::Release(const ReleaseOption &option)
{
    ...
    if (option.clearMetadata) {
        err = service->Delete(syncerParam_);
        if (err != E_OK) {
...
bool RdbStoreImpl::SetFileGid(const RdbStoreConfig &config, int32_t gid)
{
    ...
    if (!setDir) {
        LOG_ERROR("SetDbDir fail, bundleName is %{public}s, store is %{public}s.", config.GetBundleName().c_str(),
    ...
    if (!setDbFile) {
        LOG_ERROR("SetDbFile fail, bundleName is %{public}s, store is %{public}s.", config.GetBundleName().c_str(),
            SqliteUtils::Anonymous(config.GetName()).c_str());
    }
...
void RdbStoreImpl::InitDelayNotifier()
{
    ...
    delayNotifier_->SetTask(
        [param = syncerParam_, helper = helper](
...
int32_t RdbStoreImpl::ProcessOpenCallback(int version, RdbOpenCallback &openCallback)
{
    ...
    if (errCode != E_OK) {
        LOG_ERROR("openCallback failed. version: %{public}d -> %{public}d, errCode:%{public}d", currentVersion,
            version, errCode);
        return errCode;
    }
...
bool RdbStoreImpl::TryAsyncRepair()
{
    ...
    auto pool = ConnectionPool::Create(configHolder_, config_, errCode);
    if (errCode != E_OK) {
...
{
    int32_t errCode = E_OK;
    connectionPool_ = ConnectionPool::Create(configHolder_, config_, errCode);
...
int32_t RdbStoreImpl::Init(int version, RdbOpenCallback &openCallback, bool isNeedSetAcl, bool isSilentAccessible)
{
    ...
    if (config_.GetRoleType() == OWNER && !config_.IsReadOnly()) {
        errCode = InitOwner(version, openCallback);
        if (errCode != E_OK) {
            return errCode;
        }
    }
...
std::pair<int, int64_t> RdbStoreImpl::BatchInsert(const std::string &table, const ValuesBuckets &rows)
{
    ...
    auto [errCode, result] = ExecuteBatchInsert(pool, conn, executeSqlArgs, table);
    if (result == -1) {
        return { errCode, -1 };
    }
    conn = nullptr;
    int64_t insertedRows = int64_t(rows.RowSize());
    AuditInsert(table, insertedRows);
    DoCloudSync(table);
...
std::pair<int32_t, Results> RdbStoreImpl::ExecuteBatchInsertReturning(const RdbStoreImpl::BatchSqlArgs &sqlArgs,
    ...
    const Row &row, const AbsRdbPredicates &predicates, const ReturningConfig &config, Resolution resolution)
{
    ...
    AuditUpdate(predicates.GetTableName(), result.changed);
    if (result.changed > 0) {
...
std::pair<int32_t, Results> RdbStoreImpl::Delete(const AbsRdbPredicates &predicates, const ReturningConfig &config)
{
    ...
    SqlStatistic sqlStatistic("", SqlStatistic::Step::STEP_TOTAL);
    PerfStat perfStat(config_.GetPath(), "", PerfStat::Step::STEP_TOTAL);
    auto [status, sqlInfo] = RdbSqlUtils::GetDeleteSqlInfo(predicates);
    if (status != E_OK) {
        return { status, -1 };
    }
    auto returningSql = SqliteSqlBuilder::GetReturningSql(config.columns);
    auto [code, result] = ExecuteForRow(sqlInfo.sql, predicates.GetBindArgs(), config, returningSql);
    AuditDelete(predicates.GetTableName(), result.changed);
    if (result.changed > 0) {
        DoCloudSync(predicates.GetTableName());
    }
    return { code, result };
...
std::pair<int32_t, ValueObject> RdbStoreImpl::Execute(const std::string &sql, const Values &args, int64_t trxId)
{
    ...
    if (!SqliteUtils::IsSupportSqlForExecute(sqlType)) {
        LOG_ERROR("Not support the sqlType: %{public}d, app self can check the SQL:%{public}s", sqlType,
            SqliteUtils::SqlAnonymous(sql).c_str());
...
int RdbStoreImpl::SetDefaultEncryptAlgo(const ConnectionPool::SharedConn &conn, const RdbStoreConfig &config)
{
    ...
    sql = std::string(GlobalExpr::CIPHER_DEFAULT_ATTACH_KDF_ALGO_PREFIX) +
          SqliteUtils::KdfAlgoDescription(config.GetCryptoParam().kdfAlgo) + std::string(GlobalExpr::ALGO_SUFFIX);
    errCode = SetDefaultEncryptSql(statement, sql, config);
    if (errCode != E_OK) {
        return errCode;
    }
...
std::pair<int32_t, int32_t> RdbStoreImpl::Detach(const std::string &attachName, int32_t waitTime)
{
    ...
    if (statement == nullptr || errCode != E_OK) {
        ...
        SetLastErrorMsg(connection->GetLastErrorMsg());
        return { errCode, 0 };
    }
    errCode = statement->Execute(bindArgs);
    if (errCode != E_OK) {
...
int RdbStoreImpl::Commit()
{
    ...
    if (sqlStr.size() <= 1) {
        LOG_WARN("[%{public}zu, %{public}s, %{public}s]", id, SqliteUtils::Anonymous(name_).c_str(),
...
int32_t RdbStoreImpl::InitOwner(int version, RdbOpenCallback &openCallback)
{
    int32_t errCode = SetSecurityLabel(config_);
    if (errCode != E_OK) {
        return errCode;
    }
...
std::pair<int32_t, int64_t> RdbStoreImpl::ExecuteBatchInsert(const std::shared_ptr<ConnectionPool> &pool,
    ...
    if (config_.GetHaMode() != HAMode::SINGLE && SqliteUtils::IsSlaveRestoring(config_.GetPath())) {
        ...
        if (ret != E_OK) {
            if (isUseReplicaDb_) {
                ...
                return conn->CreateReplicaStatement(sql, conn, returningSql);
            }
            return { E_DATABASE_BUSY, nullptr };
        }
        ...
        auto strategy = conn->GenerateExchangeStrategy(slaveStatus_);
        LOG_WARN("Got lock file but process is not in restore, mark removed, st:%{public}d, %{public}s", strategy,
    ...
    std::vector<ValuesBucket> &&returningValues, bool isDML, int32_t rowIndex)
{
    ...
    if (code == E_OK) {
        std::shared_ptr<ResultSet> resultSet = std::make_shared<CacheResultSet>(std::move(returningValues), rowIndex);
...
void RdbStoreImpl::ReplayCallbackImpl(const RdbStoreConfig &config, std::weak_ptr<ConnectionPool> connPool)
{
    ...
    if (err != E_OK) {
        LOG_WARN(
            "get lock failed, skip binlog replay for %{public}s", SqliteUtils::Anonymous(config.GetPath()).c_str());
        return;
    }
    auto taskPool = TaskExecutor::GetInstance().GetExecutor();
    if (taskPool == nullptr) {
