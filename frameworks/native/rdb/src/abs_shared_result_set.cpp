...
int AbsSharedResultSet::GetRow(std::vector<std::string> &columns, ValuesBucket &valuesBucket)
{
    DISTRIBUTED_DATA_HITRACE(std::string(__FUNCTION__));
    ...
    for (int i = 0; i < columns.size(); ++i) {
        if (std::find(columnNames.begin(), columnNames.end(), columns[i]) != columnNames.end()) {
            ret = GetColumnIndex(columns[i], columnIndex);
            if (ret != E_OK) {
            ...
            switch (type) {
                case AppDataFwk::SharedBlock::CELL_UNIT_TYPE_FLOAT: {
                    ...
                    valuesMap.insert(std::make_pair(columns[i], ValueObject(value)));
                    break;
                }
                case AppDataFwk::SharedBlock::CELL_UNIT_TYPE_STRING: {
                    ...
                    valuesMap.insert(std::make_pair(columns[i], ValueObject(tempValue)));
                    break;
                }
                ...
                case AppDataFwk::SharedBlock::CELL_UNIT_TYPE_NULL: {
                    valuesMap.insert(std::make_pair(columns[i], ValueObject()));
                    break;
                }
