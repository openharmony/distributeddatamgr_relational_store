...
int32_t SqliteConnection::Delete(const RdbStoreConfig &config)
{
    ...
    auto binlogFolder = SqliteUtils::GetBinlogFolderPath(config);
    size_t num = SqliteUtils::DeleteFolder(binlogFolder);
    if (num > 0 && SqliteUtils::IsSupportBinlog(config)) {
        LOG_INFO("removed %{public}zu binlog related items", num);
    }
    auto slavePath = SqliteUtils::GetSlavePath(config);
    Delete(slavePath);
    Delete(path);
    return E_OK;
}
...
std::vector<std::string> SqliteConnection::GetDbFiles(const RdbStoreConfig &config)
{
    ...
    path = SqliteUtils::GetSlavePath(config);
    for (auto &suffix : FILE_SUFFIXES) {
        auto file = path + suffix.suffix_;
        struct stat fileStat;
        if (stat(file.c_str(), &fileStat) == 0) {
            // Push the full path: with a custom replicaPath the replica dir differs from
            // the master dir, so the file name alone cannot be located by the caller.
    ...
    const RdbStoreConfig &config, SlaveOpenPolicy slaveOpenPolicy)
{
    ...
    LOG_INFO("slave cfg:[%{public}d,%{public}d,%{public}d,%{public}d,%{public}d,%{public}d,%{public}d]%{public}s "
        ...
        isSlaveLockExist, hasFailure, walOverLimit, SqliteUtils::IsSupportBinlog(config_));
    if (config.GetHaMode() == HAMode::MANUAL_TRIGGER &&
        (slaveOpenPolicy == SlaveOpenPolicy::OPEN_IF_DB_VALID &&
            (!isSlaveExist || isSlaveLockExist || hasFailure || walOverLimit))) {
        if (walOverLimit) {
            SqliteUtils::SetSlaveInvalid(config_.GetPath(), SqliteUtils::SlaveInvalidReason::OPEN_WAL_OVER_LIMIT);
    ...
    if (!SqliteUtils::IsValidReplicaPath(config.GetReplicaPath())) {
        SqliteUtils::SetSlaveInvalid(config_.GetPath(), SqliteUtils::SlaveInvalidReason::OPEN_FAILED);
        LOG_WARN("invalid replicaPath, create slave connection failed:%{public}s",
            SqliteUtils::Anonymous(config.GetReplicaPath()).c_str());
        return result;
    }

    return InnerOpenSlave(config);
}

std::pair<int32_t, std::shared_ptr<SqliteConnection>> SqliteConnection::InnerOpenSlave(const RdbStoreConfig &config)
{
    std::pair<int32_t, std::shared_ptr<SqliteConnection>> result = { E_ERROR, nullptr };
    auto &[errCode, conn] = result;
    std::shared_ptr<SqliteConnection> connection = std::make_shared<SqliteConnection>(config, true, true);
    connection->SetIsSupportBinlog(SqliteUtils::IsSupportBinlog(config_));
...
{
    RdbStoreConfig rdbStoreConfig(SqliteUtils::GetSlavePath(rdbConfig));
    ...
    rdbStoreConfig.SetReplicaPath(rdbConfig.GetReplicaPath());
...
int SqliteConnection::InnerOpen(const RdbStoreConfig &config)
{
    ...
    if (isWriter_) {
        ...
        std::tie(checkErrCode, checkResult) = ExecuteForValue(sql);
        IntegrityMode auditMode = (index == INTEGRITY_QUICK_CHECK_INDEX) ? IntegrityMode::QUICK : IntegrityMode::FULL;
        int auditResult = (checkErrCode == E_OK && static_cast<std::string>(checkResult) == "ok")
        ...
        if (checkErrCode == E_OK && static_cast<std::string>(checkResult) != "ok") {
            ...
            Reportor::ReportCorruptedOnce(
                Reportor::Create(config, checkErrCode, static_cast<std::string>(checkResult)));
        }
    }
    return E_OK;
}
    ...
    std::shared_ptr<Connection> conn, sqlite3 *db, bool isFromReplica, const std::string &returningSql)
{
    ...
    if (!isFromReplica && slaveConnection_ && IsWriter() && !SqliteUtils::IsSupportBinlog(config_) &&
        ...
        if (errCode != E_OK) {
            ...
            SqliteUtils::SetSlaveInvalid(config_.GetPath(), SqliteUtils::SlaveInvalidReason::EXCHANGE_FAILED);
...
bool SqliteConnection::IsSlaveAvailable() const
{
    if (!isWriter_ || config_.GetHaMode() == HAMode::SINGLE) {
        return false;
    }
    return slaveConnection_ != nullptr && !SqliteUtils::IsSlaveInvalid(config_.GetPath());
}

void SqliteConnection::MigrateReplica(const std::string &staleSlavePath, bool isResetBinlog)
{
    LOG_INFO("migrate replica, delete stale default slave:%{public}s",
        SqliteUtils::Anonymous(staleSlavePath).c_str());
    Delete(staleSlavePath);
    if (SqliteUtils::IsSupportBinlog(config_) && config_.GetHaMode() != HAMode::SINGLE && isResetBinlog) {
...
int SqliteConnection::SetEncryptAgo(const RdbStoreConfig::CryptoParam &cryptoParam)
{
    ...
    auto errCode =
        ExecuteSql(std::string(GlobalExpr::CODEC_HMAC_ALGO_PREFIX) +
                   SqliteUtils::HmacAlgoDescription(cryptoParam.hmacAlgo) + std::string(GlobalExpr::ALGO_SUFFIX));
    if (errCode != E_OK) {
    ...
    errCode = ExecuteSql(std::string(GlobalExpr::CODEC_KDF_ALGO_PREFIX) +
                         SqliteUtils::KdfAlgoDescription(cryptoParam.kdfAlgo) + std::string(GlobalExpr::ALGO_SUFFIX));
    if (errCode != E_OK) {
        LOG_ERROR("set codec kdf algo failed, err = %{public}d", errCode);
        return errCode;
    }

    errCode = ExecuteSql(std::string(GlobalExpr::CODEC_PAGE_SIZE_PREFIX) + std::to_string(cryptoParam.cryptoPageSize));
    if (errCode != E_OK) {
...
int SqliteConnection::RegDefaultFunctions(sqlite3 *dbHandle)
{
    ...
    for (size_t i = 0; i < funcCount; i++) {
        ...
        int errCode = sqlite3_create_function_v2(dbHandle, func.name, func.numArgs, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
            nullptr, func.function, nullptr, nullptr, nullptr);
        if (errCode != SQLITE_OK) {
...
int32_t SqliteConnection::SetTokenizer(Tokenizer tokenizer)
{
    ...
    if (err != SQLITE_OK) {
        LOG_ERROR(
            "load error. err=%{public}d, errno=%{public}d, errmsg:%{public}s", err, errno, sqlite3_errmsg(dbHandle_));
    }
    ...
    std::shared_ptr<Connection> autoCheck(slaveConnection_.get(), [this, timeout](Connection *conn) {
        if (conn != nullptr && backupId_ == TaskExecutor::INVALID_TASK_ID) {
            ...
            if (rc == E_SQLITE_CORRUPT) {
                ...
                Reportor::ReportCorrupted(
                    Reportor::Create(slaveConfig, SQLiteError::ErrNo(rc), "ErrorType: slaveCheckPoint"));
...
int SqliteConnection::LimitWalSize()
{
    ...
    if (fileSize < 0 || fileSize > config_.GetWalLimitSize()) {
        ...
        ss << "The WAL file size exceeds the limit,name=" << SqliteUtils::Anonymous(walName).c_str()
           << ",file size=" << fileSize << ",limit size=" << config_.GetWalLimitSize();
    ...
    if (!isAsync) {
        if (slaveConnection_ == nullptr) {
            ...
            if (config_.GetHaMode() == HAMode::MANUAL_TRIGGER) {
                (void)SqliteUtils::MigrateBinlogFolder(config_);
            }
        }
    ...
    const bool isForceRestore)
{
    ...
    *curStatus = SlaveStatus::BACKING_UP;
    ...
    if (!isRestore && SqliteUtils::IsSupportBinlog(config_) && config_.GetHaMode() != HAMode::SINGLE) {
        (void)ResetBinlog(isNeedSetAcl);
    }
    return E_OK;
}
...
int SqliteConnection::ResetBinlog(bool isNeedSetAcl)
{
    ...
    if (IsRowForSearch()) {
        ...
        err = sqlite3_reset_hwm_binlog(dbHandle_, BinlogReadModeE::BINLOG_REPLAY_MODE);
        if (err != SQLITE_OK) {
            LOG_WARN("reset hwm binlog failed:%{public}d", err);
            return err;
        }
    } else {
        ...
        if (err != SQLITE_OK) {
            ...
            sqlite3_db_config(dbHandle_, SQLITE_DBCONFIG_ENABLE_BINLOG, nullptr);
            SqliteUtils::SetSlaveInvalid(config_.GetPath(), SqliteUtils::SlaveInvalidReason::BACKUP_VERIFY_FAILED);
        }
    }
    if (isNeedSetAcl) {
        std::string binlogDir = SqliteUtils::GetBinlogFolderPath(config_);
...
ExchangeStrategy SqliteConnection::GenerateExchangeStrategy(std::shared_ptr<SlaveStatus> status, bool isRelpay)
{
    ...
    if (SqliteUtils::IsSupportBinlog(config_) && isRelpay) {
...
int SqliteConnection::SetKnowledgeSchema(const DistributedRdb::RdbKnowledgeSchema &schema)
{
    ...
    for (const auto &table : schema.tables) {
        ...
        sourceSchema.extendColNames = std::set<std::string>(table.referenceFields.begin(), table.referenceFields.end());
...
int SqliteConnection::ExchangeVerify(bool isRestore, const bool isForceRestore)
{
    ...
    if (access(config_.GetPath().c_str(), F_OK) != 0) {
        LOG_WARN(
            "main no exist, isR:%{public}d, %{public}s", isRestore, SqliteUtils::Anonymous(config_.GetName()).c_str());
        return E_DB_NOT_EXIST;
    }
    ...
    const RdbStoreConfig &config, bool isWrite, bool isReusableReplica)
{
    ...
    if (isWrite && config.GetHaMode() != HAMode::SINGLE) {
        ...
        (void)SqliteUtils::MigrateBinlogFolder(config);
        conn->SetBinlog();
        if (!SqliteUtils::IsSupportBinlog(config)) {
            auto binlogFolder = SqliteUtils::GetBinlogFolderPath(config);
            if (access(binlogFolder.c_str(), F_OK) == 0) {
                SqliteUtils::SetSlaveInvalid(config.GetPath(), SqliteUtils::SlaveInvalidReason::STALE_BINLOG);
                size_t num = SqliteUtils::DeleteFolder(binlogFolder);
...
int SqliteConnection::VerifySlaveIntegrity(const bool isForceRestore)
{
    ...
    LOG_INFO(
        "%{public}s", SqliteUtils::FormatDebugInfoBrief(bugInfo, SqliteUtils::Anonymous(slaveCfg.GetName())).c_str());
    ...
    if (err == E_OK && (static_cast<std::string>(obj) != "ok")) {
        LOG_ERROR("slave corrupt, ret:%{public}s, cRet:%{public}d, %{public}d", static_cast<std::string>(obj).c_str(),
            err, errno);
        SqliteUtils::SetSlaveInvalid(config_.GetPath(), SqliteUtils::SlaveInvalidReason::VERIFY_FAILED);
        return E_SQLITE_CORRUPT;
    }
    return E_OK;
}
...
void SqliteConnection::BinlogOnErrFunc(void *pCtx, int errNo, char *errMsg, const char *dbPath)
{
    ...
    SqliteUtils::SetSlaveInvalid(dbPathStr, SqliteUtils::SlaveInvalidReason::BINLOG_REPLAY_FAILED);
}
...
int SqliteConnection::BinlogOpenHandle(const std::string &dbPath, sqlite3 *&dbHandle, bool isMemoryRdb)
{
    ...
    if (err != SQLITE_OK) {
        LOG_ERROR("open binlog handle error. rc=%{public}d, errno=%{public}d, p=%{public}s", err, errno,
            SqliteUtils::Anonymous(dbPath).c_str());
        sqlite3_close_v2(db);
        return E_INVALID_FILE_PATH;
    }
    dbHandle = db;
...
int SqliteConnection::CheckPathExist(const std::string &dbPath)
{
    bool isDbFileExist = access(dbPath.c_str(), F_OK) == 0;
    if (!isDbFileExist) {
        LOG_ERROR("db %{public}s not exist errno is %{public}d", SqliteUtils::Anonymous(dbPath).c_str(), errno);
        return E_DB_NOT_EXIST;
    }
    return E_OK;
}

void SqliteConnection::BinlogSetConfig(
    sqlite3 *dbHandle, Sqlite3BinlogMode binlogMode, const std::string &binlogDirPath)
{
...
Connection::ReplayCallBack SqliteConnection::GetReplayCallback(const std::string &dbPath)
{
    ...
    if (callback == nullptr) {
        LOG_WARN("replay callback expired for %{public}s", SqliteUtils::Anonymous(dbPath).c_str());
    }
    return callback;
}
...
int SqliteConnection::SetBinlog()
{
    ...
    const std::string &replicaPath = config_.GetReplicaPath();
    Sqlite3BinlogConfig binLogConfig = {
        ...
        .callbackCtx = nullptr,
        .binlogDirPath = replicaPath.empty() ? nullptr : replicaPath.c_str(),
    };
...
void SqliteConnection::ReplayBinlog(
    const std::string &dbPath, std::shared_ptr<SqliteConnection> slaveConn, bool isNeedClean)
{
    ...
    if (slaveConn->config_.GetHaMode() == HAMode::MANUAL_TRIGGER &&
        (SqliteUtils::GetFileCount(SqliteUtils::GetBinlogFolderPath(slaveConn->config_)) > BINLOG_FILE_REPLAY_LIMIT)) {
        LOG_WARN("binlog file count over limit: %{public}s", SqliteUtils::Anonymous(dbPath).c_str());
        SqliteUtils::SetSlaveInvalid(dbPath, SqliteUtils::SlaveInvalidReason::BINLOG_REPLAY_FAILED);
        return;
    }
    ...
    SqliteConnection::BinlogSetConfig(
        dbFrom, GetBinlogMode(slaveConn->config_), slaveConn->config_.GetReplicaPath());
...
void SqliteConnection::ReplayBinlog(const RdbStoreConfig &config, bool chkBinlogCount)
{
    ...
    if (chkBinlogCount && slaveConnection_->config_.GetHaMode() == HAMode::MANUAL_TRIGGER &&
        (SqliteUtils::GetFileCount(SqliteUtils::GetBinlogFolderPath(config)) > BINLOG_FILE_REPLAY_LIMIT)) {
        LOG_WARN("binlog file count over limit: %{public}s", SqliteUtils::Anonymous(config.GetPath()).c_str());
        SqliteUtils::SetSlaveInvalid(config.GetPath(), SqliteUtils::SlaveInvalidReason::BINLOG_REPLAY_FAILED);
        return;
    }
...
void SqliteConnection::DeleteCorruptSlave(const std::string &path)
{
    ...
    if (IsRowForSearch()) {
        if (dbHandle_ != nullptr) {
            ...
            if (err != SQLITE_OK) {
                ...
                err = sqlite3_reset_hwm_binlog(dbHandle_, BinlogReadModeE::BINLOG_REPLAY_MODE);
                LOG_INFO("reset hwm binlog:%{public}d", err);
            }
        }
        slaveConnection_ = nullptr;
        (void)Delete(path);
    ...
    slaveConnection_ = nullptr;
    (void)Delete(path);
    size_t num = SqliteUtils::DeleteFolder(SqliteUtils::GetBinlogFolderPath(config_), false);
    ...
    const std::pair<int, ValueObject> &sCountRes)
{
    ...
    if (SqliteUtils::IsSupportBinlog(config_) && !SqliteUtils::IsUseAsyncRestore(config_, config_.GetPath(),
        SqliteUtils::GetSlavePath(config_))) {
        SqliteConnection::ReplayBinlog(config_.GetPath(), slaveConnection_, false);
