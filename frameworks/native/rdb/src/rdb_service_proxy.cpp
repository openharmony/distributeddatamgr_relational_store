...
int32_t RdbServiceProxy::GetPromiseInfo(const RdbSyncerParam &param)
{
    MessageParcel reply;
    int32_t status = IPC_SEND(static_cast<uint32_t>(RdbServiceCode::RDB_SERVICE_CMD_GET_PROMISE_INFO), reply, param);
    if (status != RDB_OK) {
        LOG_ERROR("fail, status:%{public}d, bundleName:%{public}s, storeName:%{public}s", status,
            param.bundleName_.c_str(), SqliteUtils::Anonymous(param.storeName_).c_str());
    }
