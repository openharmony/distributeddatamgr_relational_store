...
int32_t CloudServiceProxy::NotifyChange(const std::string &eventId, const std::string &extraData, int32_t userId)
{
    MessageParcel reply;
    int32_t status = IPC_SEND(TRANS_NOTIFY_CHANGE, reply, eventId, extraData, userId);
    if (status != SUCCESS) {
