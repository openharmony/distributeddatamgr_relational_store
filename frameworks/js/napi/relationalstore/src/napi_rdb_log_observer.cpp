...
NapiLogObserver::NapiLogObserver(
...
bool NapiLogObserver::operator==(napi_value value)
{
    return JSUtils::Equal(env_, callback_, value);
}

void NapiLogObserver::OnErrorLog(const ExceptionMessage &exceptionMessage)
{
    auto queue = queue_;
    if (queue == nullptr) {
        return;
