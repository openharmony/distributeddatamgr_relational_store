...
constexpr int32_t TABLENAME_LENGTH_MAX = 256;
...
{
    if (tableName.size() > TABLENAME_LENGTH_MAX) {
        return ERR;
    }
    return OK;
}
...
{
    if (whereClause.size() > TABLENAME_LENGTH_MAX) {
        return ERR;
    }
    return OK;
}
...
int CheckValuesBucket(const ValuesBucket &valuesBucket)
{
    ...
    for (const auto &[key, val] : valuesBucket.values_) {
        if (key.size() > TABLENAME_LENGTH_MAX) {
            return ERR;
        }
    }
    return OK;
...
napi_value GetInsertSqlInfo(napi_env env, napi_callback_info info)
{
    ...
    RDB_NAPI_ASSERT(env, errcode == E_OK , std::make_shared<InnerError>(NativeRdb::E_INVALID_ARGS_NEW, GetErrorString(errcode)));
...
napi_value GetUpdateSqlInfo(napi_env env, napi_callback_info info)
{
    ...
    RDB_NAPI_ASSERT(env,
        ...
        std::make_shared<InnerError>(NativeRdb::E_INVALID_ARGS_NEW, "WhereClause is too long."));
    RDB_NAPI_ASSERT(
    ...
    RDB_NAPI_ASSERT(env,
        ...
        std::make_shared<InnerError>(NativeRdb::E_INVALID_ARGS_NEW, "The table must be not empty string."));
    auto [errcode, sqlInfo] = RdbSqlUtils::GetUpdateSqlInfo(predicates->GetTableName(),
        context->valuesBucket,
        predicates->GetWhereClause(),
        predicates->GetBindArgs(),
        conflict);
    RDB_NAPI_ASSERT(env, errcode == E_OK , std::make_shared<InnerError>(NativeRdb::E_INVALID_ARGS_NEW, GetErrorString(errcode)));
...
napi_value GetDeleteSqlInfo(napi_env env, napi_callback_info info)
{
    ...
    RDB_NAPI_ASSERT(env,
        (!predicates->GetTableName().empty()),
        std::make_shared<InnerError>(NativeRdb::E_INVALID_ARGS_NEW, "The table must be not empty string."));
    auto [errcode, sqlInfo] = RdbSqlUtils::GetDeleteSqlInfo(
        predicates->GetTableName(), predicates->GetWhereClause(), predicates->GetBindArgs());
    RDB_NAPI_ASSERT(env, errcode == E_OK , std::make_shared<InnerError>(NativeRdb::E_INVALID_ARGS_NEW, GetErrorString(errcode)));
