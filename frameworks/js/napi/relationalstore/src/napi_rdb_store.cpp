...
napi_value RdbStoreProxy::Insert(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.insert" : "Arkdata.Rdb.RdbStore.insertSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
...
napi_value RdbStoreProxy::BatchInsertWithConflictResolution(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.batchInsertWithConflictResolution"
...
napi_value RdbStoreProxy::Delete(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.delete" : "Arkdata.Rdb.RdbStore.deleteSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
...
napi_value RdbStoreProxy::Update(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.update" : "Arkdata.Rdb.RdbStore.updateSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        CHECK_RETURN_ERR(context->rdbStore != nullptr && context->rdbPredicates != nullptr);
        auto rdbStore = std::move(context->rdbStore);
...
napi_value RdbStoreProxy::Query(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.query" : "Arkdata.Rdb.RdbStore.querySync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        CHECK_RETURN_ERR(context->rdbStore != nullptr && context->rdbPredicates != nullptr);
#if defined(CROSS_PLATFORM)
        context->resultSet = context->StealRdbStore()->QueryByStep(*(context->rdbPredicates), context->columns, false);
#else
...
napi_value RdbStoreProxy::QuerySql(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.querySql" : "Arkdata.Rdb.RdbStore.querySqlSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        RdbStoreProxy *obj = reinterpret_cast<RdbStoreProxy *>(context->boundObj);
...
napi_value RdbStoreProxy::ExecuteSql(napi_env env, napi_callback_info info)
{
    ...
    auto input = [context](napi_env env, size_t argc, napi_value *argv, napi_value self) {
        ...
        if (argc == 2) {
            CHECK_RETURN(OK == ParseBindArgs(env, argv[1], context));
            CHECK_RETURN_SET_E(
                !RdbSqlUtils::HasDuplicateAssets(context->bindArgs), std::make_shared<ParamError>("Duplicate assets "
                                                                                                  "are not allowed"));
        }
    };
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.executeSql" : "Arkdata.Rdb.RdbStore.executeSqlSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
...
napi_value RdbStoreProxy::Execute(napi_env env, napi_callback_info info)
{
    ...
    auto input = [context](napi_env env, size_t argc, napi_value *argv, napi_value self) {
        ...
        if (argc == 2) {
            ...
            if (type == napi_number) {
                ...
                CHECK_RETURN(OK == ParseBindArgs(env, argv[1], context));
                CHECK_RETURN_SET_E(!RdbSqlUtils::HasDuplicateAssets(context->bindArgs),
                    std::make_shared<ParamError>("Duplicate assets are not allowed"));
            }
        }
        if (argc == 3) {
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.execute" : "Arkdata.Rdb.RdbStore.executeSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto status = E_ERROR;
...
napi_value RdbStoreProxy::Replace(napi_env env, napi_callback_info info)
{
    ...
    auto input = [context](napi_env env, size_t argc, napi_value *argv, napi_value self) {
        ...
        CHECK_RETURN(OK == ParseTableName(env, argv[0], context));
        CHECK_RETURN(OK == ParseValuesBucket(env, argv[1], context));
        CHECK_RETURN_SET_E(!RdbSqlUtils::HasDuplicateAssets(context->valuesBucket),
            std::make_shared<ParamError>("Duplicate assets are not allowed"));
    };
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.replace" : "Arkdata.Rdb.RdbStore.replaceSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        int errCode =
...
napi_value RdbStoreProxy::Backup(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto res = context->StealRdbStore()->Backup(context->tableName, context->newKey);
...
napi_value RdbStoreProxy::Attach(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto res = context->StealRdbStore()->Attach(
...
napi_value RdbStoreProxy::RollBackByTxId(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        ...
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        return context->StealRdbStore()->RollBack(context->txId);
    };
    auto output = [context](napi_env env, napi_value &result) {
        napi_status status = napi_get_undefined(env, &result);
        CHECK_RETURN_SET_E(status == napi_ok, std::make_shared<InnerError>(E_ERROR));
...
napi_value RdbStoreProxy::Commit(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        ...
        return context->StealRdbStore()->Commit(context->txId);
    };
    auto output = [context](napi_env env, napi_value &result) {
        napi_status status = napi_get_undefined(env, &result);
        CHECK_RETURN_SET_E(status == napi_ok, std::make_shared<InnerError>(E_ERROR));
...
napi_value RdbStoreProxy::QueryWithoutRowCount(napi_env env, napi_callback_info info)
{
    DISTRIBUTED_DATA_HITRACE(std::string(__FUNCTION__));
    auto context = std::make_shared<RdbStoreContext>();
    auto input = [context](napi_env env, size_t argc, napi_value *argv, napi_value self) {
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.queryWithoutRowCount" : "Arkdata.Rdb.RdbStore.queryWithoutRowCountSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM, true);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto rdbStore = std::move(context->rdbStore);
...
napi_value RdbStoreProxy::QuerySqlWithoutRowCount(napi_env env, napi_callback_info info)
{
    DISTRIBUTED_DATA_HITRACE(std::string(__FUNCTION__));
    auto context = std::make_shared<RdbStoreContext>();
    auto input = [context](napi_env env, size_t argc, napi_value *argv, napi_value self) {
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.querySqlWithoutRowCount" : "Arkdata.Rdb.RdbStore.querySqlWithoutRowCountSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM, true);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto rdbStore = std::move(context->rdbStore);
        DistributedRdb::QueryOptions options{.preCount = false, .isGotoNextRowReturnLastError = true};
...
napi_value RdbStoreProxy::Restore(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        ...
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        return context->StealRdbStore()->Restore(context->srcName, context->newKey);
    };
    auto output = [context](napi_env env, napi_value &result) {
        napi_status status = napi_get_undefined(env, &result);
        CHECK_RETURN_SET_E(status == napi_ok, std::make_shared<InnerError>(E_ERROR));
...
napi_value RdbStoreProxy::SetLocale(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto errCode = context->StealRdbStore()->ConfigLocale(context->srcName);
        return errCode;
    };
...
struct RetainDeviceDataContext : public EnhancedContext {
    int32_t Parse(napi_env env, size_t argc, napi_value *argv, napi_value self)
    {
        ASSERT_RETURN_SET_ERROR(argc < 2, std::make_shared<ParamNumError>("0 - 1"));
        RdbStoreProxy *obj = GetNativeInstance(env, self);
        ...
        if (argc == 1) {
            auto result = JSUtils::Convert2Value(env, argv[0], retainDevices, true);
            ASSERT_RETURN_SET_ERROR(
...
{
    auto context = std::make_shared<RetainDeviceDataContext>();
    auto input = [context](napi_env env, size_t argc, napi_value *argv, napi_value self) {
...
napi_value RdbStoreProxy::Sync(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [queue, defer, callback, predicates, rdbStore = context->StealRdbStore(),
        ...
        HistogramReporter guard("Arkdata.Rdb.RdbStore.sync",
...
napi_value RdbStoreProxy::CleanDirtyData(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        ...
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        return context->StealRdbStore()->CleanDirtyData(context->tableName, context->cursor);
    };
...
napi_value RdbStoreProxy::ModifyLockStatus(napi_env env, napi_callback_info info, bool isLock)
{
    ...
    auto exec = [context, isLock]() -> int {
        CHECK_RETURN_ERR(context->rdbStore != nullptr && context->rdbPredicates != nullptr);
        return context->StealRdbStore()->ModifyLockStatus(*(context->rdbPredicates), isLock);
    };
    auto output = [context](napi_env env, napi_value &result) {
        napi_status status = napi_get_undefined(env, &result);
        CHECK_RETURN_SET_E(status == napi_ok, std::make_shared<InnerError>(E_ERROR));
...
napi_value RdbStoreProxy::LockCloudContainer(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        ...
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto result = context->StealRdbStore()->LockCloudContainer();
...
napi_value RdbStoreProxy::UnlockCloudContainer(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() {
        ...
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        return context->StealRdbStore()->UnlockCloudContainer();
    };
    auto output = [context](napi_env env, napi_value &result) {
        auto status = napi_get_undefined(env, &result);
        CHECK_RETURN_SET_E(status == napi_ok, std::make_shared<InnerError>(E_ERROR));
...
napi_value RdbStoreProxy::CreateTransaction(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.createTransaction" : "Arkdata.Rdb.RdbStore.createTransactionSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
...
napi_value RdbStoreProxy::BatchInsertWithReturning(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.batchInsertWithReturning"
            : "Arkdata.Rdb.RdbStore.batchInsertWithReturningSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM, true);
        CHECK_RETURN_ERR(context->rdbStore != nullptr);
        auto result = context->StealRdbStore()->BatchInsert(context->tableName, context->valuesBuckets,
            context->config, context->conflictResolution);
        context->result = result.second;
        guard.SetErrCode(result.first);
        return result.first;
    };
    auto output = [context](napi_env env, napi_value &result) {
...
struct RdbUpdateWithReturningContext : public RdbContext {
    int32_t Parse(napi_env env, size_t argc, napi_value *argv, napi_value self)
    {
        // the parameters are either 3 or 4.
...
napi_value RdbStoreProxy::UpdateWithReturning(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.updateWithReturning"
            : "Arkdata.Rdb.RdbStore.updateWithReturningSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM, true);
        CHECK_RETURN_ERR(context->rdbStore != nullptr && context->rdbPredicates != nullptr);
        auto result = context->StealRdbStore()->Update(
...
struct RdbDeleteWithReturningContext : public RdbContext {
    int32_t Parse(napi_env env, size_t argc, napi_value *argv, napi_value self)
    {
...
napi_value RdbStoreProxy::DeleteWithReturning(napi_env env, napi_callback_info info)
{
    ...
    auto exec = [context]() -> int {
        const char *hName = context->isAsync_
            ? "Arkdata.Rdb.RdbStore.deleteWithReturning"
            : "Arkdata.Rdb.RdbStore.deleteWithReturningSync";
        HistogramReporter guard(hName, HistogramType::TIME | HistogramType::ENUM, true);
        CHECK_RETURN_ERR(context->rdbStore != nullptr && context->rdbPredicates != nullptr);
        auto result = context->StealRdbStore()->Delete(*(context->rdbPredicates), context->config);
        context->result = result.second;
        guard.SetErrCode(result.first);
        return result.first;
    };
