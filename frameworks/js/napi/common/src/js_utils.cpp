...
napi_value JSUtils::Convert2JSValue(napi_env env, const DistributedRdb::Statistic &statistic)
{
    napi_value jsValue;
...
napi_value JSUtils::Convert2JSValue(napi_env env, const DistributedRdb::TableDetail &tableDetail)
{
    napi_value jsValue;
    napi_status status = napi_create_object(env, &jsValue);
    if (status != napi_ok) {
...
napi_value JSUtils::Convert2JSValue(napi_env env, const DistributedRdb::TableDetails &tableDetails)
{
    napi_value jsValue;
...
napi_value JSUtils::Convert2JSValue(napi_env env, const DistributedRdb::ProgressDetail &progressDetail)
{
    napi_value jsValue;
    napi_status status = napi_create_object(env, &jsValue);
    if (status != napi_ok) {
...
napi_value JSUtils::Convert2JSValue(napi_env env, const DistributedRdb::Details &details)
{
    return nullptr;
}
