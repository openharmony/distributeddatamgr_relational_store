...
template<typename T>
int32_t Convert2Value(napi_env env, napi_value jsValue, std::map<std::string, T> &value, bool isEmptyMap = false);
...
template<typename T>
int32_t JSUtils::Convert2Value(napi_env env, napi_value jsValue, std::map<std::string, T> &value, bool isEmptyMap)
{
    ...
    if (isEmptyMap && jsCount == 0 && status == napi_ok) {
        return napi_ok;
    }
