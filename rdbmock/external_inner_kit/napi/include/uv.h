...
struct uv_statfs_s {
  ...
  uint64_t f_ffree;
  uint64_t f_spare[4];
  // NOTICE: we don't follow the community on the OHOS platform.
};
...
UV_EXTERN int uv_register_task_to_event(struct uv_loop_s* loop,
                                        uv_post_task func,
                                        uv_pending_higher_event_checker checker);
...
/* Unicode utilities needed internally for dealing with Windows. */
